...
function JSEL(data, config, empty, text){
    ...
    setChild(data, parent.id, child.id, child.val, empty, text);
    document.getElementById(parent.id).addEventListener('change', function(){
        setChild(data, parent.id, child.id, null, empty, text);
    });
}
...
function setChild(data, parent_id, child_id, child_val, empty, text){
 ...
 *            sortJSEL
 * =============================
 * JSON配列データを元に親セレクトボックスの値から子セレクトボックスのオプションを切り替える
 * @param object data jsonデータ
 * @param object config 設定 
                            { parent: {id: 親のid, val: 親の初期値}, 
                              child:  {id: 子のid, val: 子の初期値}  }
 ...
 * @param string text セレクトボックスの空項目に表示するテキスト
 */
function sortJSEL(data, config, empty, text){
    var parent = config.parent;
    var child  = config.child;
    sortSetSelect(data, parent.id, parent.val, empty, text);
    sortSetChild(data, parent.id, child.id, child.val, empty, text);
    document.getElementById(parent.id).addEventListener('change', function(){
        sortSetChild(data, parent.id, child.id, null, empty, text);
    });
}
function sortSetSelect(data, parent_id, parent_val, empty, text){
    var parent  = initElement(parent_id, text);
...
function sortSetChild(data, parent_id, child_id, child_val, empty, text){
    var parent    = document.getElementById(parent_id);
    var child     = initElement(child_id, text);
...
add(function() {
  ...
  for (var i = 0; elem.length > i; i++){
      ...
      loadJson(data.path, sortJSEL, config, empty, text);
  }
});